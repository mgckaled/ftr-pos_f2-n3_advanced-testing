...
describe("Address Value Object", () => {
  describe("Constructor", () => {
    ...
    it("deve lançar erro quando rua estiver vazia", () => {
      expect(
        () => new Address("", 123, "São Paulo", "SP", "01234-567")
      ).to.throw("Address fields cannot be empty");
    });

    it("deve lançar erro quando cidade estiver vazia", () => {
      expect(
        () => new Address("Rua das Flores", 123, "", "SP", "01234-567")
      ).to.throw("Address fields cannot be empty");
    });

    it("deve lançar erro quando estado estiver vazio", () => {
      expect(
        () => new Address("Rua das Flores", 123, "São Paulo", "", "01234-567")
      ).to.throw("Address fields cannot be empty");
    });

    it("deve lançar erro quando CEP estiver vazio", () => {
      expect(
        () => new Address("Rua das Flores", 123, "São Paulo", "SP", "")
      ).to.throw("Address fields cannot be empty");
    });

    it("deve lançar erro quando número for negativo", () => {
      expect(
        () => new Address("Rua das Flores", -1, "São Paulo", "SP", "01234-567")
      ).to.throw("Number must be a positive integer");
    });

    it("deve lançar erro quando número for zero", () => {
      expect(
        () => new Address("Rua das Flores", 0, "São Paulo", "SP", "01234-567")
      ).to.throw("Number must be a positive integer");
    });
  });
